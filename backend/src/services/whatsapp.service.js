const getWhatsappConfig = () => {
  ...
  if (!phoneNumberId) throw new Error("Falta WHATSAPP_PHONE_NUMBER_ID en el .env");
  return { token, phoneNumberId, apiVersion };
};

const sendPayload = async (to, payload) => {
  ...
  return { ...data, messageId, phone: normalizedPhone };
};
...
export const normalizeWhatsappPhone = (phone) => {
  ...
  if (normalized.startsWith("15") && normalized.length <= 10) normalized = normalized.slice(2);
  if (!normalized.startsWith("54")) normalized = `54${normalized}`;
  if (normalized.startsWith("54") && !normalized.startsWith("549")) normalized = `549${normalized.slice(2)}`;
  if (normalized.length < 12 || normalized.length > 15) return null;
  return normalized;
};

export const sendWhatsappTextMessage = async (to, message) => {
  if (!message || !String(message).trim()) throw new Error("El mensaje de WhatsApp está vacío");
  return sendPayload(to, {
    type: "text",
    text: { preview_url: false, body: String(message).trim() },
  });
};
